...
module "email_communication" {
  ...
  communication_service_name = "comms-${var.project_name}-${var.environment}"
  email_service_name         = "email-${var.project_name}-${var.environment}"
  resource_group_name        = module.resource_group.name
  tags = {
    environment = var.environment
    managed_by  = "terraform"
...
resource "random_password" "postgresql_administrator" {
  length           = 32
  special          = true
  override_special = "!#$%&*+-=?@^_"

  keepers = {
    version = var.postgresql_administrator_password_version
  }
}
...
module "postgresql" {
  ...
  administrator_password         = random_password.postgresql_administrator.result
...
resource "azurerm_key_vault_secret" "database_url" {
  ...
  value_wo         = "postgresql://${module.postgresql.administrator_login}:${urlencode(random_password.postgresql_administrator.result)}@${module.postgresql.fqdn}:5432/${module.postgresql.database_name}?sslmode=require"
...
module "notification_function" {
  ...
  storage_account_name              = "st${var.project_name}function${var.environment}"
  service_plan_name                 = "asp-${var.project_name}-notifications-${var.environment}"
  application_insights_name         = "appi-${var.project_name}-notifications-${var.environment}"
  location                          = var.location
  resource_group_name               = module.resource_group.name
  log_analytics_workspace_id        = module.log_analytics.id
...
moved {
  from = azurerm_postgresql_flexible_server_firewall_rule.backend_container_app
  to   = azurerm_postgresql_flexible_server_firewall_rule.backend_container_app[0]
}
