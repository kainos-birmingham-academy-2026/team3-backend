...
export class JobRoleMapper {
	...
	statusToResponse(
		rows: { statusId: number; statusName: string }[],
	): StatusResponse[] {
	...
	capabilityToResponse(
		rows: { capabilityId: number; capabilityName: string }[],
	): CapabilityResponse[] {
		return rows.map(
			(row) => new CapabilityResponse(row.capabilityId, row.capabilityName),
		);
	}

	locationToResponse(
		rows: { locationId: number; locationName: string }[],
	): LocationResponse[] {
		return rows.map(
			(row) => new LocationResponse(row.locationId, row.locationName),
		);
	}
}
